...
class MongooseModel extends Abstract {
  ...
  populate = (query: Query<any, any>) =>
    query
      .populate('_workspace', '_id name coordinates location timezone ownerId members');
  ...
  isUserHasPermission = async (deviceId: string, userId: Types.ObjectId | string, roles?: Roles[]): Promise<[boolean, string?]> => {
    ...
    const isOwner = device?._workspace?.ownerId.equals(userId);
    const hasPermission = (
      member &&
      permission &&
      !!roles &&
      roles.some(role => permission?.role?.includes(role))
    )
    const isAuthorized =
      isOwner ||
      (
  ...
  findOnlyIds = async (workspaceId: Types.ObjectId | string): Promise<Types.ObjectId[]> => {
    const devices = await this.find<IDeviceModelWithId>(
